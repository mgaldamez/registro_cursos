...
Class Usuario{
	...
	function viewDetCur($idUser){
		$sql = "select c.crs_nombre, c.crs_jornada, DATE_FORMAT(c.crs_ini,'%d %b %Y') AS crs_ini, DATE_FORMAT(c.crs_fin,'%d %b %Y') AS crs_fin, p.pat_nombre from tb_cursos c join tb_patrocinadores p on c.crs_patrocinador = p.pat_id  where c.crs_id in (select curso_id from tb_part_curso where empleado_id=$idUser)";
		$rs=$this->conn->execute($sql);
